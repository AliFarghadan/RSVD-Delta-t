...
PetscErrorCode PermuteMat(Mat F_hat, PetscInt Nw, Mat F_hat_re)
{
	...
	for (ik=0; ik<k; ik++) {
		for (iw=0; iw<Nw; iw++) {
			ii   = ik    + iw*k;
			jj   = ik*Nw + iw;
			ierr = MatDenseGetColumnVecRead(F_hat,ii,&V1);CHKERRQ(ierr);
			ierr = MatDenseGetColumnVecWrite(F_hat_re,jj,&V2);CHKERRQ(ierr);
			ierr = VecCopy(V1,V2);CHKERRQ(ierr);
			ierr = MatDenseRestoreColumnVecWrite(F_hat_re,jj,&V2);CHKERRQ(ierr);
			ierr = MatDenseRestoreColumnVecRead(F_hat,ii,&V1);CHKERRQ(ierr);
		}
	}
