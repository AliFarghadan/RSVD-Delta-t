...
PetscErrorCode CreateRandomMat(RSVD_matrices *RSVD_mat, RSVDt_vars *RSVDt, Directories *dirs)
{
	...
	*/  
	...
	if (flg_Fin) {
		ierr = PetscSNPrintf((char*)&dirs->IO_dir,PETSC_MAX_PATH_LEN,"%s%s",dirs->RootDir,dirs->filename);CHKERRQ(ierr);
		...
		// ierr = PetscPrintf(PETSC_COMM_WORLD,"Reading the forcing matrix from previously obtained RSVD-delta-t!\n");CHKERRQ(ierr);
		...
		// 	ierr = PetscSNPrintf((char*)&dirs->IO_dir,PETSC_MAX_PATH_LEN,"%s%s%s%d%s",dirs->RootDir,dirs->ResultsDir,"twin_resolvent_modes/V_hat_twin_k",(int) ik+1,"_allFreqs");CHKERRQ(ierr);
		// 	ierr = PetscViewerBinaryOpen(PETSC_COMM_WORLD,dirs->IO_dir,FILE_MODE_READ,&fd);CHKERRQ(ierr);
		...
		ierr = PetscPrintf(PETSC_COMM_WORLD,"Random forcing is generated!\n");CHKERRQ(ierr);
		ierr = MatSetSizes(RSVD_mat->Y_hat,PETSC_DECIDE,PETSC_DECIDE,RSVDt->RSVD.N,RSVDt->RSVD.Nw_eff*RSVDt->RSVD.k);CHKERRQ(ierr);
		ierr = MatSetUp(RSVD_mat->Y_hat);CHKERRQ(ierr);
