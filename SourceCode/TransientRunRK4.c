...
#include "EfficientTransientRemovalEstimation.h"
#include "DestroyTSMats.h"

PetscErrorCode TransientRunRK4(TransientRun_vars *TR_vars, RSVDt_vars *RSVDt, LNS_vars *LNS_mat, DFT_matrices *DFT_mat, Directories *dirs)
{
	/*
	...
	*/
	...
	// RSVDt->TS.flg_dir_adj = TR_vars->TransientRun_dir;
	...
	*/
	...
	ierr = CreateResultsDir(dirs, "Transient_snapshots_");CHKERRQ(ierr); 
	...
	*/
	...
	Nt_period        = TSS/RSVDt->TS.dt;
	Nt_saved_delta   = Nt_period/Ns;
	rend             = Nt_period*TR_vars->Periods;
	Nt_saved         = PetscFloorReal(rend/TR_vars->mod)+1;
	ierr = PetscPrintf(PETSC_COMM_WORLD,"Total integration time = %g = %d x %g\n", RSVDt->TS.dt*rend, (int) TR_vars->Periods, TSS);CHKERRQ(ierr);

	/*
		Creates the required matrices/vecs
	*/

	ierr = TR_vars->TransientSave ? PetscPrintf(PETSC_COMM_WORLD,"Total snapshots to be saved = %d\n", (int) Nt_saved) : \
										PetscPrintf(PETSC_COMM_WORLD,"Saving snapshots is not requested\n");CHKERRQ(ierr);  

	if (TR_vars->EffTransTest) {
	...
	*/
	...
	if (flg_IC) {
		...
		ierr = PetscSNPrintf((char*)&dirs->IO_dir,PETSC_MAX_PATH_LEN,"%s%s%s",dirs->root_dir,dirs->output,dirs->filename);CHKERRQ(ierr);
		...
		ierr = PetscRandomSetSeed(r, TR_vars->Rseed);CHKERRQ(ierr);
	...
	*/
		...
		*/
		...
		if (TR_vars->EffTransTest) {
			if (PetscFmodReal(i,Nt_saved_delta) == 0) {
				...
				if (pos == 0) ierr = EfficientTransientRemovalEstimation(Q_all, qss, i/Nt_period, RSVDt, DFT_mat, dirs);CHKERRQ(ierr);
	...
	/*
		Saves the norm of snapshots throughout the intergation (every mod number)
	...
	*/ 
	...
	ierr = VecAssemblyBegin(trans_norm);CHKERRQ(ierr);
	ierr = VecAssemblyEnd(trans_norm);CHKERRQ(ierr);
	ierr = PetscSNPrintf((char*)&dirs->IO_dir,PETSC_MAX_PATH_LEN,"%s%s",dirs->FolderDir,"q_transient_norms.h5");CHKERRQ(ierr);
    ...
    // ierr = PetscViewerHDF5SetBaseDimension2(fd, PETSC_FALSE);CHKERRQ(ierr); // Set chunk size to 1x839
	ierr = PetscObjectSetName((PetscObject)trans_norm, "trans_norm");CHKERRQ(ierr);
	...
	/*
		Removes the variables from memory and return
	*/

	ierr = VecDestroy(&q0);CHKERRQ(ierr);
	if (TR_vars->EffTransTest) ierr = MatDestroy(&Q_all);CHKERRQ(ierr);
	if (TR_vars->EffTransTest) ierr = VecDestroy(&qss);CHKERRQ(ierr);
