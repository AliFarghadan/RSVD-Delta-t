...
PetscErrorCode CreateMtilde(Mat Y_all, LNS_vars *LNS, RSVDt_vars *RSVDt, TS_removal_matrices *TSR)
{
	/*
	...
	*/  
	...
	ierr = MatDuplicate(V,MAT_DO_NOT_COPY_VALUES,&MV);CHKERRQ(ierr);
	...
	for (ik=0; ik<RSVDt->RSVD.k; ik++) {
		...
		ierr = TSExpADeltaT(LNS, RSVDt, MV);CHKERRQ(ierr);
