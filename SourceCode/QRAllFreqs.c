...
PetscErrorCode QRAllFreqs(RSVD_matrices *RSVD, RSVDt_vars *RSVDt)
{
	/*
	...
	*/  
	...
	for (iw=0; iw<RSVDt->RSVD.Nw_eff; iw++) {
		ierr = MatDenseGetSubMatrix(RSVD->Y_hat,PETSC_DECIDE,PETSC_DECIDE,iw*RSVDt->RSVD.k,(iw+1)*RSVDt->RSVD.k,&Y_temp);CHKERRQ(ierr);
		...
		ierr = MatDenseRestoreSubMatrix(RSVD->Y_hat,&Y_temp);CHKERRQ(ierr);
	...
	*/
	...
	if (RSVDt->Display) ierr = PetscPrintf(PETSC_COMM_WORLD,"*** QR decomposition elapsed time = %02d:%02d:%02d ***\n", (int)hh, (int)mm, (int)ss);CHKERRQ(ierr);
