...
- Note that the order of frequencies is as follows: column 1 corresponds to frequency 0, column 2 to frequency $\omega$, and so on up to frequency $\omega_{max}$. After this, the frequencies continue from $\omega_{min}$ up to the last column, which represents the $-\omega$ frequency. This ordering is the same as that obtained from MATLAB's FFT function. For example, when $N_{\omega} = 42$, the frequencies will be ordered as $0, \omega, 2\omega, ... 20\omega, -21\omega, -20\omega, ..., -\omega$. If the operator is real-valued, only the positive frequencies are retained.
