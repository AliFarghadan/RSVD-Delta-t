...
- Note that the order of frequencies is as follows: column 1 corresponds to frequency 0, column 2 to frequency $\omega$, and so on up to frequency $\left(\frac{N_{\omega}}{2} - 1 \right) \times \omega$. After this, the frequencies continue from $-\frac{N_{\omega}}{2} \times \omega$ up to the last column, which represents the $-\omega$ frequency. This ordering is the same as that obtained from MATLAB's FFT function. For example, when $N_{\omega} = 42$, the frequencies will be ordered as $0, \omega, 2\omega, ... 20\omega, -21\omega, -20\omega, ..., -\omega$.
...
- `InputMatrixDir`: Defines the path to the intput matrix weight $(B)$ directory.
...
- Once the computation is complete, two saving formats are available:
  ...
  - **Option 2 when `SaveResultsOpt = 2`:**
    ...
    - The order of frequencies starts with column 1 (frequency 0), column 2 (frequency $\omega$), up to frequency $\frac{N_{\omega}}{2} \times \omega$, and then from $-\left(\frac{N_{\omega}}{2} + 1\right) \times \omega$ up to the last column that contains the $-\omega$ frequency (equivalent to MATLAB ordering). For example, when $N_{\omega} = 42$, the frequencies will be ordered as $0, \omega, 2\omega, ... 20\omega, -21\omega, -20\omega, ..., -\omega$.
